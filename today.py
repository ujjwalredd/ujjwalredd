...
def daily_readme(joined):
    ...
    """
    diff = relativedelta.relativedelta(datetime.datetime.today(), joined)
    return '{} {}, {} {}, {} {}'.format(
...
def stars_counter(data):
    ...
    for node in data:
        ...
        stargazers = repo.get('stargazers') if isinstance(repo, dict) else None
...
def svg_overwrite(filename, age_data, commit_data, star_data, repo_data, contrib_data, follower_data, loc_data):
    ...
    Only the value text is replaced -- the surrounding dot-fill is static in the template so the
    left-aligned value column never shifts as numbers grow or shrink.
    """
...
if __name__ == '__main__':
    ...
    acc_joined = datetime.datetime.strptime(acc_date, '%Y-%m-%dT%H:%M:%SZ')
    age_data, age_time = perf_counter(daily_readme, acc_joined)
